...
def test_add_new_data_source(setup_and_teardown):
    ...
    result = uganda_data_sources.add_new_data_source(new_source, "twitter_account")
    assert result
...
def test_get_all_data_sources(setup_and_teardown):
    uganda_data_sources = DataSources('test_data_sources.json')
    accounts = uganda_data_sources.get_all_sources()
    assert accounts == ["account1", "account2", "account3"]


def test_filter_data_sources(setup_and_teardown):
    uganda_data_sources = DataSources('test_data_sources.json')
    filtered_sources = uganda_data_sources.filter_data_sources(type="news")
    ...
    filtered_sources = uganda_data_sources.filter_data_sources(type="news", bias="neutral")
    assert len(filtered_sources) == 1